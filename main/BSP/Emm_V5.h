...
void Motion_Control(uint16_t Left_Velocity, uint16_t Right_Velocity, uint32_t Left_Distance, uint32_t Right_Distance); //双轮运动
void Straight_Forward(uint16_t velocity, uint32_t distance); //直线运动
void Turn(uint8_t direction, uint8_t angle, uint16_t velocity); //原地转弯

void process_received_data(uint8_t *data, uint8_t length); //处理树莓派串口信息
#endif
